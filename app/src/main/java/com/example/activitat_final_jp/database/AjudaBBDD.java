...
public class AjudaBBDD extends SQLiteOpenHelper {
    ...
    private final String CREATE_TABLE_MSG = "create table " + TAB_MSG + "("
            ...
            + "FOREIGN KEY ("+ COL_MSG_ID_USER +") REFERENCES " + TAB_USER + "(" + COL_USER_ID + "))";

    public AjudaBBDD(Context context)
    {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.w(AjudaBBDD.class.getName(), "Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
